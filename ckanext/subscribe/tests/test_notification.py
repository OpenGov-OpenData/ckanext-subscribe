...
class TestGetImmediateNotifications(object):
    ...
    def test_basic(self):
        ...
        assert notifies.keys(), [subscription['email']]
        assert _get_activities(notifies), [('bob@example.com', 'new package', dataset['id'])]
    ...
    def test_old_activity_not_notified(self):
        ...
        assert not _get_activities(notifies)
    ...
    def test_activity_before_the_subscription_is_not_notified(self):
        ...
        assert _get_activities(notifies), [('bob@example.com', 'changed package', dataset['id'])]
    ...
    def test_lots_of_users_and_datasets(self):
        ...
        assert set(_get_activities(notifies)), {(
...
class TestSendWeeklyNotificationsIfItsTimeTo(object):
    ...
    def test_basic(self, send_notification_email):
        ...
        assert [(a['activity_type'], a['data']['package']['id'])
                for a in notifications[0]['activities']], [('new package', dataset['id'])]
        assert notifications[0]['subscription']['id'], subscription['id']
        assert time_since_emails_last_sent(Frequency.WEEKLY.value) < datetime.timedelta(seconds=1)
...
class TestSendDailyNotificationsIfItsTimeTo(object):
    ...
    def test_basic(self, send_notification_email):
        ...
        assert [(a['activity_type'], a['data']['package']['id'])
                for a in notifications[0]['activities']], [('new package', dataset['id'])]
        assert notifications[0]['subscription']['id'], subscription['id']
        assert time_since_emails_last_sent(Frequency.DAILY.value) < datetime.timedelta(seconds=1)
...
class TestMostRecentWeeklyNotification(object):
    ...
    def test_earlier_that_week(self):
        assert most_recent_weekly_notification_datetime(
               datetime.datetime(2020, 1, 25)), datetime.datetime(2020, 1, 24, 9, 0)  # a saturday

    def test_later_that_week(self):
        assert most_recent_weekly_notification_datetime(
               datetime.datetime(2020, 1, 23)), datetime.datetime(2020, 1, 17, 9, 0)  # a thursday

    def test_same_day_of_week_earlier_in_the_day(self):
        assert most_recent_weekly_notification_datetime(
               datetime.datetime(2020, 1, 24, 8, 0)), datetime.datetime(2020, 1, 17, 9, 0)  # a friday

    def test_same_day_of_week_later_in_the_day(self):
        assert most_recent_weekly_notification_datetime(
               datetime.datetime(2020, 1, 24, 10, 0)), datetime.datetime(2020, 1, 24, 9, 0)  # a friday
